    ...
    <>
      ...
      <nav className="z-50 fixed top-0 left-0 w-full flex flex-col px-3 md:px-8 lg:px-15 py-3 border-b border-gray-300 bg-white/90 backdrop-blur-md">
        <div className="flex items-center justify-between w-full">
          ...
          {/* Desktop Search Bar */}
          <div className="hidden lg:flex flex-1 mx-4 items-center text-sm gap-2 border border-gray-300 px-3 rounded-full max-w-xl">
            <input
          ...
          <div className="hidden lg:flex items-center gap-8">
            ...
            {!user ? (
              <div className="flex flex-col gap-2 items-start">
                <button onClick={() => setShowUserLogin(true)} className="flex items-center justify-center h-6 px-4 py-1.5 bg-primary hover:bg-primary-dull transition text-white rounded-full text-sm border-2">Login</button>
                <button onClick={() => navigate('/seller')} className="flex items-center justify-center h-6 px-4 py-1.5 bg-primary hover:bg-primary-dull transition text-white rounded-full text-sm border-2">Seller</button>
              </div>
            ) : (
      ...
      <div className="sm:hidden fixed bottom-3 left-1/2 -translate-x-1/2 w-[95%] max-w-md rounded-2xl px-3 py-2 flex justify-between items-center bg-white/30 backdrop-blur-md border border-gray-300 z-50 shadow-xl">
        ...
        <button onClick={() => handleNavigate('/products')} className="flex flex-col items-center text-xs text-gray-700 hover:text-primary">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" className="w-6 h-6 mb-1 text-gray-600" fill="currentColor">
        ...
        <button onClick={() => handleNavigate('/contact')} className="flex flex-col items-center text-xs text-gray-700 hover:text-primary">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6 mb-1 text-gray-600">
